	...
	MyButton_start("START")
{
	...
	set_size_request(785,480); //végsõõõ(800,440)
	set_border_width(30);
	add(MyTable);
...
void MyWindow::game(){
	...
	t1="'s turn, locate circle.",t2="'s turn, locate cross.";
...
*/
	print_label.set_text(p1);
	MyFrame.add(print_label);
...
void MyWindow::circle_or_cross(){
	if(c==0){
		status_label.set_text(p1);//lehet, hogy cserélni kell... attól függ, mit rak, és mi a kiírás.
	}
	if(c==1){
		status_label.set_text(p2);
	}
	MyToggleButton::my_click();
	c=abs((c%2)-1);
}
